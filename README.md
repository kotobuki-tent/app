...
- **入口の身元確認**（2026-09-17〜、まだ記録だけ）：`window.fetch` を包んで API 呼び出し全部に tok（LINE WORKS WOFF トークン）／key（`?k=…` で1回入れる端末の合言葉）／cid（端末の印）を自動付与。GAS の `authCheck_` が照合して `auth_log` シートに記録。拒否はスクリプトプロパティ `AUTH_ENFORCE=true` で切替
