...
- 送った変更の控え（2026-09-17）：貸出/予約・メンテ行の保存・削除は既存 ID のまま書くので、取り直し（8秒後・自動更新）で GAS が未反映だとサーバーの古い値に戻ってしまっていた。`Inv` の `fireAndForget` が `save_rental`/`delete_rental` を控え（`_pend`）に取り、`loadAll` の `applyPending` で「サーバーに反映済み→控えを消す／15秒経っても未反映→1回だけ再送／60秒で諦めて警告トースト」。ガントもメンテもこの `rentals` を描くので、片方で変えればもう片方も即時に一致する
