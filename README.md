...
別途、`register.html`（スマホ特化の商品追加専用ツール）と `alcohol.html`（アルコールチェック記録＝安全運転管理者制度対応）は単独ページとして独立配置。SPA本体とは別に、それぞれ専用URL／ホーム画面アイコンで配布。
...
| `alcohol_checks` | アルコールチェック記録（道交法・安全運転管理者制度）。運転者×日×運転前/後。確認者/車両/検知器使用/対面区分/酒気帯び有無/指示事項。`alcohol.html` から記録、1年保存 |
...
- **Google Apps Script**：API v7、deptパラメータで部門振り分け（production / project / sales / inventory / overtime / vehicle / daily / labor / attendance / drive / factory / alcohol）。GASソースの正本は `Code.gs`（iCloud `♿️SEQUENCE LAB/`）。工数アーカイブは dept=labor の POST（archive/unarchive）で `labor_archived` を切替
