...
- **実機反映は最大10分**（GitHub Pages CDN cache）＋ SW が spa.html を stale-while-revalidate でキャッシュしている（2026-09-03〜）ので**開き直し1回分遅れて反映**（1回目の開き直しで裏取得→2回目で新版）。「更新」ボタンはデータ再取得のみ。開きっぱなしの端末には 10 分以内に「新しい版があります」が出て、10 分放置なら自動で読み直す（2026-09-17〜、`spa/ver.txt` をフックが書く）。
